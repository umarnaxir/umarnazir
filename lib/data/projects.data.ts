import { Project } from '../types';

export const projectsData: Project[] = [
  {
    id: '1',
    number: '01',
    year: '2026',
    category: 'EDUCATION / COACHING',
    title: 'Exceptional IAS Academy',
    description:
      'Coaching institute website for civil services aspirants featuring course listings, faculty profiles, batch enrollment, and student resources. Built with a clean, conversion-focused layout and SEO-optimized, responsive design.',
    tags: ['NEXT.JS', 'TYPESCRIPT', 'SEO', 'RESPONSIVE', 'NODE.JS'],
    link: 'https://exceptionaliasacademy.vercel.app/',
    imageLink: '/images/exceptional.png',
    visualVariant: 'alt',
    note: 'This project was completed under the supervision of NexGen Developers.',
  },
  {
    id: '2',
    number: '02',
    year: '2026',
    category: 'E-COMMERCE / RETAIL',
    title: 'ShoesHub',
    description:
      'E-commerce platform for footwear retail with product catalog, search and filtering, wishlist, cart, and secure checkout. Delivers a polished shopping experience with a high-performance, responsive design.',
    tags: ['NEXT.JS', 'TYPESCRIPT', 'E-COMMERCE', 'MONGODB', 'NODE.JS', 'RESPONSIVE'],
    link: 'https://shoeshubpoonch.vercel.app/',
    imageLink: '/images/shoeshub.png',
    visualVariant: 'alt2',
    note: 'This project was completed under the supervision of NexGen Developers.',
  },
  {
    id: '3',
    number: '03',
    year: '2026',
    category: 'FITNESS / GYM',
    title: 'FitSpace Gym',
    description:
      'Fitness and gym website showcasing membership plans, training programs, trainer profiles, and class schedules. Features a bold, energetic layout with SEO-optimized, responsive design and lead-capture forms.',
    tags: ['NEXT.JS', 'TYPESCRIPT', 'SEO', 'RESPONSIVE', 'NODE.JS'],
    link: 'https://fitspacegym.vercel.app/',
    imageLink: '/images/fitspacegym.png',
    visualVariant: 'default',
    note: 'This project was completed under the supervision of NexGen Developers.',
  },
  {
    id: '4',
    number: '04',
    year: '2026',
    category: 'EDUCATION / LIBRARY',
    title: 'Citadel Library',
    description:
      'Library management and study-space website featuring seat booking, membership plans, resource listings, and timings. Built with a clean, modern layout and SEO-optimized, responsive design.',
    tags: ['NEXT.JS', 'TYPESCRIPT', 'SEO', 'BOOKING SYSTEM', 'RESPONSIVE'],
    link: 'https://citadellibrary.vercel.app/',
    imageLink: '/images/citadil.png',
    visualVariant: 'alt2',
    note: 'This project was completed under the supervision of NexGen Developers.',
  },
  {
    id: '5',
    number: '05',
    year: '2026',
    category: 'AGENCY / SOFTWARE DEVELOPMENT',
    title: 'NexGen Developers',
    description:
      'Software development agency website showcasing digital products, web and mobile applications, and growth-focused solutions. Built with a modern, SEO-optimized stack and a high-performance, conversion-driven design.',
    tags: ['NEXT.JS', 'TYPESCRIPT', 'SEO', 'NODE.JS', 'RESPONSIVE'],
    link: 'https://nexgendevelopers.in/',
    imageLink: '/images/nexgen-developers.png',
    visualVariant: 'alt',
    note: 'This project was completed under the supervision of NexGen Developers.',
  },
  {
    id: '6',
    number: '06',
    year: '2026',
    category: 'AGENCY / DIGITAL SOLUTIONS',
    title: 'Saphiam',
    description:
      'Corporate website for a technology company delivering custom software, web, and digital solutions. Features a clean service-led layout, case studies, and SEO-optimized, responsive design for strong online presence.',
    tags: ['NEXT.JS', 'TYPESCRIPT', 'SEO', 'NODE.JS', 'RESPONSIVE'],
    link: 'https://saphiam.com/',
    linkLabel: 'Visit Website',
    imageLink: '/images/saphiam.png',
    visualVariant: 'alt2',
    note: 'This project was completed under the supervision of NexGen Developers.',
  },
  {
    id: '7',
    number: '07',
    year: '2026',
    category: 'AGENCY / DIGITAL SOLUTIONS',
    title: 'Vipthra Technologies',
    description:
      'Digital solutions agency website built to drive growth through web development, branding, and online marketing. Delivers a polished, conversion-focused experience with SEO-optimized, responsive design.',
    tags: ['NEXT.JS', 'TYPESCRIPT', 'SEO', 'DIGITAL MARKETING', 'RESPONSIVE'],
    link: 'https://vipthra.com/',
    imageLink: '/images/vipthra-mockup.png',
    visualVariant: 'default',
    note: 'This project was completed under the supervision of NexGen Developers.',
  },
  {
    id: '8',
    number: '08',
    year: '2025',
    category: 'TOURISM / BOOKING',
    title: 'NineMash',
    description:
      'Tourism & Online Booking Platform for Kashmir region. Enables users to book hotels, tours, and experiences with seamless payment integration and real-time availability.',
    tags: ['NEXT.JS', 'TYPESCRIPT', 'MONGODB', 'SEO', 'NODE.JS', 'AWS'],
    link: 'https://ninemash.com/',
    imageLink: '/images/ninemash.png',
    visualVariant: 'alt2',
    note: 'This project was completed under the supervision of Saibbyweb.',
  },
  {
    id: '9',
    number: '09',
    year: '2026',
    category: 'AI / CONSTRUCTION INTELLIGENCE',
    title: 'Sitesynai',
    description:
      'Construction Intelligence Powered by AI. Our AI-powered platform combines cutting-edge technology with industry expertise to deliver the most comprehensive plan analysis solution available. AI assists your team, highlighting discrepancies so you can focus on decisions.',
    tags: ['NEXT.JS', 'TYPESCRIPT', 'AI/ML', 'CONSTRUCTION', 'PLAN ANALYSIS', 'NODE.JS'],
    link: 'https://sitesyncai.vercel.app/',
    imageLink: '/images/sitesync.png',
    visualVariant: 'alt2',
    note: 'This project was completed under the supervision of Saibbyweb.',
  },
  {
    id: '10',
    number: '10',
    year: '2025',
    category: 'AI / ML PLATFORM',
    title: 'AI-Mployed',
    description:
      'AI/ML Talent & Career Platform connecting job seekers with opportunities in artificial intelligence and machine learning. Built with React.js and Next.js for optimal performance and user experience.',
    tags: ['NEXT.JS', 'TYPESCRIPT', 'AI/ML', 'MONGODB', 'NODE.JS', 'AWS'],
    link: 'https://ml-jobs.com',
    imageLink: '/images/mljobs.png',
    visualVariant: 'default',
    note: 'This project was completed under the supervision of Saibbyweb.',
  },
  {
    id: '11',
    number: '11',
    year: '2026',
    category: 'ENTERPRISE / HR',
    title: 'Office Management System',
    description:
      'HR & Workflow Automation system for managing employee data, attendance, payroll, and workflow processes. Features role-based access control, reporting dashboards, and automated notifications.',
    tags: ['REACT', 'NODE.JS', 'FIREBASE', 'MONGODB', 'AUTOMATION', 'HR SYSTEM'],
    link: 'https://sw-office.vercel.app/',
    imageLink: '/images/office-management.png',
    visualVariant: 'alt',
    note: 'This project was completed under the supervision of Saibbyweb.',
  },
  {
    id: '12',
    number: '12',
    year: '2025',
    category: 'HEALTHCARE / PORTFOLIO',
    title: 'Dr. Jibran Bashir',
    description:
      'Professional portfolio website for Orthopedic Surgeon featuring appointment booking, patient testimonials, and medical service information with responsive design and SEO Executive.',
    tags: ['NEXT.JS', 'TYPESCRIPT', 'SEO', 'RESPONSIVE', 'MONGODB','NODE.JS'],
    link: 'https://www.drjibranbashir.com/',
    imageLink: '/images/dr-jibran-bashir.png',
    visualVariant: 'default',
    note: 'This project was completed under the supervision of NexGen Developers.',
  },
  {
    id: '13',
    number: '13',
    year: '2025',
    category: 'LOGISTICS / E-COMMERCE',
    title: 'EliteExpress Courier',
    description:
      'Delivery & Logistics System for managing shipments, tracking packages, and optimizing delivery routes. Features real-time tracking, automated notifications, and comprehensive admin dashboard.',
    tags: ['NEXT.JS', 'NODE.JS', 'MONGODB', ],
    link: 'https://eliteexpressdeliveryservices.com/',
    imageLink: '/images/eliteexpress.png',
    visualVariant: 'alt',
    note: 'This project was completed under the supervision of Saibbyweb.',
  },
  {
    id: '14',
    number: '14',
    year: '2024',
    category: 'TRAVEL / BOOKING',
    title: 'Kashmir Tickets',
    description:
      'Travel & Ticket Booking System for Kashmir region. Comprehensive platform for booking flights, buses, and travel packages with secure payment processing and booking management.',
    tags: ['NEXT.JS', 'SEO', 'MONGODB'],
    link: 'https://kashmirtickets.com/',
    imageLink: '/images/kashmirtickets.png',
    visualVariant: 'alt',
    note: 'This project was completed under the supervision of Harmukh Technologies.',
  },
  {
    id: '15',
    number: '15',
    year: '2025',
    category: 'LOGISTICS / COURIER',
    title: 'Rush Expected Courier',
    description:
      'All Critical Shipments courier service platform. Specialized in handling urgent and time-sensitive deliveries with real-time tracking, priority booking, and customer support integration.',
    tags: ['REACT',  'REAL-TIME TRACKING', 'FIREBASE', 'PYTHON'],
    link: 'https://re-courier.com/',
    imageLink: '/images/re-courier.png',
    visualVariant: 'default',
    note: 'This project was completed under the supervision of Harmukh Technologies.',
  },
  {
    id: '16',
    number: '16',
    year: '2024',
    category: 'DIGITAL MARKETING / AGENCY',
    title: 'Digitlia',
    description:
      'Digital marketing agency specializing in web and app development and SEO. Showcases services, portfolio, client testimonials, and case studies with integrated contact forms and lead generation features.',
    tags: ['REACT', 'NEXT.JS', 'SEO', 'DIGITAL MARKETING', 'LEAD GENERATION', 'PYTHON'],
    link: 'https://digitlia.com/',
    imageLink: '/images/digitlia.jpeg',
    visualVariant: 'alt',
    note: 'This project was completed under the supervision of Harmukh Technologies.',
  },
  {
    id: '17',
    number: '17',
    year: '2025',
    category: 'NGO / NON-PROFIT',
    title: 'Kindness Towards Humanity',
    description:
      'NGO Website for humanitarian organization. Features donation system, volunteer registration, event management, and impact stories with multi-language support and accessibility features.',
    tags: ['REACT', 'NEXT.JS', 'DONATION SYSTEM', 'ACCESSIBILITY'],
    link: 'https://kindnesstowardshumanity.in/',
    imageLink: '/images/kindnesstowardshumanity.png',
    visualVariant: 'alt2',
    note: 'This project was completed under the supervision of NexGen Developers.',
  },
  {
    id: '18',
    number: '18',
    year: '2024',
    category: 'HOSPITALITY / BOOKING',
    title: 'Hotel Sea View',
    description:
      'Hospitality Booking Website for hotel reservations. Features room availability, online booking, payment processing, and guest management system with beautiful UI/UX design.',
    tags: ['REACT', 'NEXT.JS', 'BOOKING SYSTEM', 'NODE.JS'],
    link: 'https://thehotelseaview.in/',
    imageLink: '/images/hotel-sea-view.png',
    visualVariant: 'alt2',
    note: 'This project was completed under the supervision of NexGen Developers.',
  },
  {
    id: '19',
    number: '19',
    year: '2024',
    category: 'HEALTHCARE / MEDICAL',
    title: 'Altaf Hospital',
    description:
      'Healthcare & Medical Services Website for hospital. Includes appointment booking, doctor profiles, department information, and patient portal with secure data handling and HIPAA compliance considerations.',
    tags: ['NEXT.JS', 'TYPESCRIPT', 'SEO', 'RESPONSIVE', 'MONGODB','NODE.JS'],
    link: 'https://www.altafhospital.com/',
    imageLink: '/images/altaf-hospital.png',
    visualVariant: 'default',
    note: 'This project was completed under the supervision of Harmukh Technologies.',
  },
];
